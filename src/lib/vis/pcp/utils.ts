...
export function pcpMergeDefaultConfig(
    ...
    if (merged.numColumnsSelected.length === 0 && columns.length > 1) {
        ...
        if (
            columns[columns.length - 1].info.id !==
...
export async function createPCPTraces(
    ...
    if (!config.numColumnsSelected || !config.catColumnsSelected) {
        return {
            ...
            rows: 0,
            cols: 0,
            errorMessage: 'tdp:core.vis.pcpError'
        };
    }
            ...
            dimensions: [
                ...numColValues.map((c, i) => {
                    return {
                        range: [
                            d3.min(
                                c.resolvedValues.map(v => v.val) as number[]
                            ),
                ...
                ...catColValues.map(c => {
                    const uniqueList = [
                        ...new Set<string>(
                            c.resolvedValues.map(v => v.val) as string[]
                        )
                    ];
                    ...
                    return {
                        ...
                        label: c.info.name,
                        values: c.resolvedValues.map(curr =>
                            uniqueList.indexOf(curr.val as string)
                        ),
