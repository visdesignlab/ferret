...
export async function createBarTraces(
    ...
    if (!config.catColumnsSelected) {
        return {
            ...
            rows: 0,
            cols: 0,
            errorMessage: 'tdp:core.vis.whatever_lol'
        };
    }

    const plots: PlotlyData[] = [];

    const catCols: VisCategoricalColumn[] = columns.filter(
        c =>
            config.catColumnsSelected.some(d => c.info.id === d.id) &&
            c.type === EColumnTypes.CATEGORICAL
    ) as VisCategoricalColumn[];

    if (catCols.length > 0) {
        if (config.group && config.multiples) {
            plotCounter = await setPlotsWithGroupsAndMultiples(
                columns,
                catCols,
                config,
                plots,
            ...
            plotCounter = await setPlotsWithGroups(
                columns,
                catCols,
                config,
                plots,
            ...
            plotCounter = await setPlotsWithMultiples(
                columns,
                catCols,
                config,
                plots,
            ...
            plotCounter = await setPlotsBasic(
                columns,
                catCols,
                config,
                plots,
    ...
    return {
        ...
        rows,
        cols,
        errorMessage: 'tdp:core.vis.barError'
    };
}
...
async function setPlotsWithGroupsAndMultiples(
    ...
    const currGroupColumn = await resolveSingleColumn(
        getCol(columns, config.group)
    );
    const currMultiplesColumn = await resolveSingleColumn(
        getCol(columns, config.multiples)
    );

    const uniqueGroupVals: string[] = [
        ...new Set(currGroupColumn.resolvedValues.map(v => v.val))
    ] as string[];
    const uniqueMultiplesVals: string[] = [
        ...new Set(currMultiplesColumn.resolvedValues.map(v => v.val))
    ] as string[];
    ...
    uniqueMultiplesVals.forEach(uniqueMultiples => {
        uniqueGroupVals.forEach(uniqueGroup => {
            const groupedLength = uniqueColVals.map(v => {
                const allObjs = (
                    catCurr.resolvedValues as VisCategoricalValue[]
                )
                    .filter(c => c.val === v)
                    .map(c => c.id);
                const allGroupObjs = (
                    currGroupColumn.resolvedValues as VisCategoricalValue[]
                )
                    .filter(c => c.val === uniqueGroup)
                    .map(c => c.id);
                const allMultiplesObjs = (
                    currMultiplesColumn.resolvedValues as VisCategoricalValue[]
                )
                    .filter(c => c.val === uniqueMultiples)
                    .map(c => c.id);

                const joinedObjs = allObjs.filter(
                    c =>
                        allGroupObjs.includes(c) && allMultiplesObjs.includes(c)
                );

                return normalizedFlag
                    ? (joinedObjs.length / allObjs.length) * 100
                    : joinedObjs.length;
            });
            ...
            plots.push({
                data: {
                    x: vertFlag
                        ? [...new Set(catCurr.resolvedValues.map(v => v.val))]
                        : groupedLength,
                    y: !vertFlag
                        ? [...new Set(catCurr.resolvedValues.map(v => v.val))]
                        : groupedLength,
...
async function setPlotsWithGroups(
    ...
    const normalizedFlag = config.display === EBarDisplayType.NORMALIZED;
    const catCurr = catColValues[0];
    const groupColumn = await resolveSingleColumn(
        getCol(columns, config.group)
    );

    const uniqueGroupVals: string[] = [
        ...new Set(groupColumn.resolvedValues.map(v => v.val))
    ] as string[];
    const uniqueColVals: string[] = [
        ...new Set(catCurr.resolvedValues.map(v => v.val))
    ] as string[];

    uniqueGroupVals.forEach(uniqueVal => {
        const groupedLength = uniqueColVals.map(v => {
            const allObjs = (catCurr.resolvedValues as VisCategoricalValue[])
                .filter(c => c.val === v)
                .map(c => c.id);
            const allGroupObjs = (
                groupColumn.resolvedValues as VisCategoricalValue[]
            )
                .filter(c => c.val === uniqueVal)
                .map(c => c.id);
...
async function setPlotsWithMultiples(
    ...
    const uniqueGroupVals: string[] = [
        ...new Set((await multiplesColumn).resolvedValues.map(v => v.val))
    ] as string[];
    const uniqueColVals: string[] = [
        ...new Set(catCurr.resolvedValues.map(v => v.val))
    ] as string[];
    ...
    uniqueGroupVals.forEach(uniqueVal => {
        const groupedLength = uniqueColVals.map(v => {
            const allObjs = (catCurr.resolvedValues as VisCategoricalValue[])
                ...
                .map(c => c.id);
            const allGroupObjs = (
                multiplesColumn.resolvedValues as VisCategoricalValue[]
            )
                .filter(c => c.val === uniqueVal)
                .map(c => c.id);
            const joinedObjs = allObjs.filter(c => allGroupObjs.includes(c));
...
async function setPlotsBasic(
    ...
    const count = [...new Set(catCurr.resolvedValues.map(v => v.val))].map(
        curr =>
            (catCurr.resolvedValues as VisCategoricalValue[]).filter(
                c => c.val === curr
            ).length
    );
