...
export function BarVis({
    ...
    const {
        ...
        status: traceStatus,
        error: traceError
    } = useAsync(createBarTraces, [columns, config, scales]);
    ...
    const layout = React.useMemo(() => {
        ...
        const layout: Plotly.Layout = {
            ...
            violingap: 0,
            barmode:
                config.groupType === EBarGroupingType.STACK ? 'stack' : 'group'
        };
                    ...
                    <div className="container pb-3" style={{ width: '20rem' }}>
                        ...
                        {mergedOptionsConfig.multiples.enable
                            ? mergedOptionsConfig.multiples.customComponent || (
                                  <MultiplesSelect
                                      callback={(multiples: ColumnInfo) =>
                                          setConfig({ ...config, multiples })
                                      }
                                      columns={columns}
                        ...
                        {mergedOptionsConfig.direction.enable
                            ? mergedOptionsConfig.direction.customComponent || (
                                  <BarDirectionButtons
                                      callback={(direction: EBarDirection) =>
                        ...
                        {mergedOptionsConfig.groupType.enable
                            ? mergedOptionsConfig.groupType.customComponent || (
                                  <BarGroupTypeButtons
                                      callback={(groupType: EBarGroupingType) =>
                                          setConfig({ ...config, groupType })
                                      }
                        ...
                        {mergedOptionsConfig.display.enable
                            ? mergedOptionsConfig.display.customComponent || (
                                  <BarDisplayButtons
                                      callback={(display: EBarDisplayType) =>
