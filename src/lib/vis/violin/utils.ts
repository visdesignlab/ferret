...
export async function createViolinTraces(
    ...
    if (!config.numColumnsSelected || !config.catColumnsSelected) {
        return {
            ...
            rows: 0,
            cols: 0,
            errorMessage: 'tdp:core.vis.violinError'
        };
    }

    const _numCols: VisNumericalColumn[] = columns.filter(
        c =>
            config.numColumnsSelected.some(d => c.info.id === d.id) &&
            c.type === EColumnTypes.NUMERICAL
    ) as VisNumericalColumn[];
    const _catCols: VisCategoricalColumn[] = columns.filter(
        c =>
        ...
        for (const numCurr of numColValues) {
            plots.push({
                data: {
                    ...
                    box: {
                        visible:
                            config.violinOverlay === EViolinOverlay.BOX
                                ? true
                                : false
                    },
                    meanline: {
            ...
            plots.push({
                data: {
                    ...
                    transforms: [
                        {
                            type: 'groupby',
                            groups: catCurr.resolvedValues.map(
                                v => v.val
                            ) as string[],
                            styles: [
                                ...new Set<string>(
                                    catCurr.resolvedValues.map(
                                        v => v.val
                                    ) as string[]
                                )
                            ].map(c => {
                                return {
                                    target: c,
                                    value: { line: { color: scales.color(c) } }
                                };
                            })
                        }
                    ]
